...
int main() {
	...
	for(int i = 0; i < 3; i++) {
		person[i].name.setFirstName("Markus");
		person[i].name.setLastName("Stenqvist");
		person[i].address.setStreet("Lugna Gatan 3");
		person[i].address.setZipcode("12345");
		person[i].address.setCity("Stockholm");
		person[i].setPersNr("900513");
		person[i].setSkoNr(43);
	}
	int x = 1, choice = 0;
	while(x == 1) {
		// Print the menu
...
void printPersons(Person *obj) {
	cout << "*** Persons ***" << endl;
	for(int i = 0; i < (signed) sizeof(obj); i++) {
...
void setPerson(Person *person) {
	...
	for(i = 0; i < (signed) sizeof(person); i++) {
